...
def get_teacher(device, teacher_c, hpo_lr:float, trunc:int):
    ...
    optimizer_teacher = MetaOptimizer(meta_optimizier, hpo_lr=hpo_lr, 
                                      truncate_iter=trunc, max_grad_norm=10)
...
def main(dir:Path, hpo_lr:float, trunc:int, ipc:int, epochs:int, mx:int):
    for i in range(3): # думаю 3-х будет достаточно!
        loop_c, teacher_c = set_config(ipc, epochs)
        ...
        for epoch in tqdm(range(1, loop_c.epochs + 1)):
            acc = loop.ift_train(max_steps=mx)
...
if __name__ == "__main__":
    ...
    usual_args = {'hpo_lr':0.01, 'trunc':10, 'ipc':10, 'epochs': 1_000, 'mx':10}
    iargs = [('ipc', el, ep) for el, ep in zip([60, 50, 10], [250, 250, 1070])]
    margs = [('mx', el, ep) for el, ep in zip([1, 50],  [1440, 500] )]
    targs = [('trunc', el, ep) for el, ep in zip([5, 30], [1560, 500])]
    args = [{nm: el, 'epochs': ep,
             'dir': common_dir/f'{nm}_{el}'} for nm, el, ep in 
             iargs + margs + targs]
    args = [{**usual_args, **arg} for arg in args]
    for arg in args:
        print(arg)
