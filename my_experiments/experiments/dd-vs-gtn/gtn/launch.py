...
if __name__ == "__main__":
    ...
    for k in [64, 128]: # 16, 32, 64, 128
      usual_args = {'bsz':100, 'ous':10, 'ls':10, 'ic':1, 'teacher_k':k}
      bargs = [('bsz', el, ep) for el, ep in zip([500, 100], [50, 110])] # 1_000, 35
      args = [{nm: el, 'epoch': ep,
               'dir': common_dir/f'{nm}_{el}_k{k}'} for nm, el, ep in bargs]
      args = [{**usual_args, **arg} for arg in args]
      for arg in args:
          print(arg)
          if JUST_TO_CHECK:
            arg['dir'] = Path(str(arg['dir'])+'check')
