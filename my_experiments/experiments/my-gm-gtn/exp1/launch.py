...
def main(bsz:int, dir:Path, epoch:int, ic:int,
         ous:int, ls:int, rs:int):
    for i in range(3): # думаю 3-х будет достаточно!
        ...
        for epoch in tqdm(range(1, loop_c.epochs + 1)):
            acc = loop.my_gmpc_train(outer_steps=ous, 
                                     learner_steps=ls, 
                                     batch_size=256,
                                     real_steps=rs)
...
if __name__ == "__main__":
    ...
    usual_args = {'bsz':100, 'ous':10, 'ls':10, 'ic': 1, 'rs': 1}
    iargs = [('rs', el, ep) for el, ep in zip([2, 5, 10], [60, 30, 15])]
    args = [{nm: el, 'epoch': ep,
             'dir': common_dir/f'{nm}_{el}'} for nm, el, ep in iargs]
