...
def set_config(bsz:int, ic:int, epochs:int):
...
def main(bsz:int, dir:Path, epoch:int, ic:int, ous:int, ls:int):
    for i in range(3): # думаю 3-х будет достаточно!
        loop_c, teacher_c = set_config(bsz, ic, epoch)
...
if __name__ == "__main__":
    ...
    usual_args = {'bsz':100, 'ous':10, 'ls':10, 'ic': 1}
    bargs = [('bsz', el, ep) for el, ep in zip([100, 500, 1_000], [110, 50, 35])]
    largs = [('ls', el, ep) for el, ep in zip([5, 50], [110, 50])]
    oargs = [('ous', el, ep) for el, ep in zip([5, 50], [180, 30])]
    args = [{nm: el, 'epoch': ep,
