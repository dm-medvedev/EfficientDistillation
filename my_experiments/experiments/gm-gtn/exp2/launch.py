...
def main(bsz:int, ls: int, dir:Path):
    for i in range(3): # думаю 3-х будет достаточно!
        loop_c, teacher_c = set_config(bsz)
        ...
        for epoch in tqdm(range(1, loop_c.epochs + 1)):
            acc = loop.gmpc_train(outer_steps=10, 
                                  learner_steps=ls, 
                                  batch_size=256)
...
if __name__ == "__main__":
    ...
    for bsz in [100, 1_000]:
      for ls in [2, 5, 10]:
        dir = common_dir/f'B_{bsz}_ls_{ls}'
